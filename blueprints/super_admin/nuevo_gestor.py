...
from db import ejecutar_query, ejecutar_non_query       # 🔌 Funciones propias para consultar/ejecutar SQL
...
# =====================================================
@nuevo_gestor_bp.route("/nuevo_gestor", methods=["GET", "POST"])  # 🔗 Ruta /super_admin/nuevo_gestor
def nuevo_gestor():
    """
    ...
    # -------------------------------------------------
    if request.method == "POST":                        # 📥 Solo entra aquí si el usuario ha enviado el formulario
        ...
        password_plana = request.form.get("password", "").strip()     # 🔑 Contraseña en claro (solo mientras se procesa)

        idtbl_tipos_de_vias = request.form.get("idtbl_tipos_de_vias") # 🛣️ FK tipo de vía (puede ser None)
        ...
        extension = request.form.get("extension", "").strip() or None # ☎ Extensión (None si viene vacío)

        # ✅ Campo 'activo' puede llegar como "on" (checkbox) o como "1"
        activo_raw = request.form.get("activo")                       # 📥 Valor crudo del checkbox
        activo = 1 if activo_raw in ("on", "1") else 0                # ✅ Normalizamos a 1 (activo) o 0 (inactivo)

        idtbl_roles = request.form.get("idtbl_roles")                 # 🎭 Rol seleccionado en el desplegable
        numero_profesional = request.form.get("numero_profesional", "").strip()  # 🧾 Nº profesional colegiado

        # 🔐 must_change (si debe cambiar la contraseña al primer login)
        must_change_str = request.form.get("must_change", "1")        # 📥 Valor como texto ("1" o "0"), por defecto "1"
        try:
        ...
        if not nombre or not email or not password_plana:             # ❗ Comprobamos mínimos
            flash("Nombre, email y contraseña son obligatorios.", "danger")  # 💬 Aviso al usuario
        ...
        ).decode("utf-8")                                             # 🔁 Guardamos el hash como texto en la BD
        ...
        try:
            ...
            flash("Gestor creado correctamente.", "success")         # 💬 Mensaje de éxito a la interfaz
            ...
            flash(f"Error creando gestor: {e}", "danger")            # 💬 Mostramos error al usuario
