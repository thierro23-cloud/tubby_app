...
def obtener_endpoints_por_blueprint(blueprint_name: str) -> List[Dict[str, Any]]:
    ...
    endpoints_filtrados = [
        ep for ep in todos_los_endpoints
        if ep['endpoint'].startswith(f'{blueprint_name}.') or blueprint_name in ep['endpoint']
    ]
    
    return endpoints_filtrados


def formatear_endpoints_para_tabla(endpoints: List[Dict[str, Any]]) -> List[Dict[str, str]]:
    """
