...
def get_db_config():
    ...
    """
    import os
    db_config = os.environ.get("DB_CONFIG", "host=localhost dbname=mydatabase user=myuser password=mypassword")
    return db_config
...
def sync_to_database(endpoints, logger):
    ...
    try:
        ...
        logger.info(f"✓ Sincronización de endpoints completa.  Insertados: {result['inserted']}, Actualizados: {result['updated']}, Desactivados: {result['deactivated']}")
