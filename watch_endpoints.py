...
def setup_logging():
    ...
    file_formatter = logging.Formatter(
        WatcherConfig.LOG_FORMAT, 
        WatcherConfig.LOG_DATE_FORMAT
    )
...
class EndpointFileWatcher(FileSystemEventHandler):
    ...
    def _sync_endpoints(self):
        ...
        try:
            ...
            self.logger.error(
                f"✗ Excepción durante sincronización: {str(e)}",
                exc_info=True
            )
...
def main():
    ...
    try:
        ...
        observer.schedule(
            event_handler, 
            path=project_path, 
            recursive=WatcherConfig.WATCH_RECURSIVE
        )
        observer.start()
        
        logger.info(f"  ✓ Vigilando: {project_path}")
        logger.info(f"  ✓ Modo recursivo: {'Sí' if WatcherConfig.WATCH_RECURSIVE else 'No'}")
