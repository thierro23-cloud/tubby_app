...
def sync_obras():  # 🧩 Esta función es la que hace toda la magia de sincronizar
    ...
    try:  # 🧩 Intentamos hacer todo esto
        my_cur = my_conn.cursor(
            dictionary=True
        )  # 🧩 Creamos un cursor que devuelve filas como diccionario
        ...
        obras = (
            my_cur.fetchall()
        )  # 🧩 Guardamos todas las obras en una lista llamada obras
        ...
        for obra in obras:  # 🧩 Recorremos cada obra de la lista
            ...
            id_gis = obra[
                "idtbl_gis_municipal"
            ]  # 🧩 Cogemos el id que apunta a GIS (si existe)
            ...
            if id_gis is None:  # 🧩 Si la obra aún NO tiene id GIS
                ...
                new_id = pg_cur.fetchone()[
                    "id"
                ]  # 🧩 Leemos el id nuevo que creó obras_gis
                ...
                my_cur.execute(  # 🧩 Actualizamos la obra en MySQL
                    ...
                    (
                        new_id,
                        idtbl_obras,
                    ),  # 🧩 Ponemos el id GIS nuevo en la fila de esa obra
                )
                ...
                pg_cur.execute(  # 🧩 Mandamos una orden de UPDATE a PostgreSQL
                    ...
                    """,
                    {
                        **obra,
                        "id": id_gis,
                    },  # 🧩 Enviamos todos los datos de la obra y el id GIS actual
                )
...
if (
    __name__ == "__main__"
):  # 🧩 Si ejecutamos este archivo directamente desde la consola
    sync_obras()  # 🧩 Llamamos a la función que sincroniza todas las obras
    print(
        "Sincronización obras → obras_gis completada."
    )  # 🧩 Mostramos un mensajito al terminar
